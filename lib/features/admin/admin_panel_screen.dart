...
import '../../services/incremental_sync_service.dart' as inc;
...
class _AdminPanelScreenState extends ConsumerState<AdminPanelScreen>
  ...
  Future<void> _uploadQrCode() async {
    try {
      ...
      final String? downloadUrl = await CloudinaryService.uploadImageUnsigned(
        imageFile,
        fileName,
      );
  ...
  Future<void> _handleSongsRefresh() async {
    try {
      ...
      _showSnackbar('Songs refreshed successfully!', isError: false);
    } catch (e) {
      _showSnackbar('Failed to refresh songs: $e');
    }
  }
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              await _performDelete(type, item);
            },
  ...
  Widget _buildSongsTab() {
    ...
    final filteredSongs = ref.watch(adminFilteredSongsProvider);
    final syncStatus = ref.watch(adminSyncStatusProvider);

    return RefreshIndicator(
      onRefresh: () async {
        await _handleSongsRefresh();
      },
            ...
            children: [
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Using cached data • ${syncStatus['active']} active • ${syncStatus['deleted']} deleted',
                              style: GoogleFonts.inter(
                                fontSize: 12,
                                color: theme.colorScheme.primary,
                              ),
              ...
              Expanded(
                child: songsAsync.when(
                  data: (songs) {
                    final displaySongs = filteredSongs.where((song) => !song.isDeleted).toList();
                          ...
                          child: Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  ref.watch(adminSongSearchProvider).isEmpty ? 'No songs available' : 'No songs found',
                                  style: GoogleFonts.inter(
                                    fontSize: 18,
                                    fontWeight: FontWeight.w600,
                            ...
                            subtitle: Row(
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: _getChangeTypeColor(song.changeType).withValues(alpha: 0.2),
                                    borderRadius: BorderRadius.circular(4),
                                  ),
                                  child: Text(
                                    song.changeType.toUpperCase(),
                                    style: GoogleFonts.inter(
                                      fontSize: 10,
                                      fontWeight: FontWeight.bold,
                                      color: _getChangeTypeColor(song.changeType),
                                    ),
                                  ),
                                ),
                              ],
                  ...
                  error: (e, s) => Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text('Error loading songs', style: GoogleFonts.inter(color: theme.colorScheme.error)),
                        const SizedBox(height: 8),
                        ElevatedButton(
                          onPressed: _handleSongsRefresh,
  ...
  Widget _buildScheduleTab() {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        ref.invalidate(adminSchedulesProvider);
        _showSnackbar('Schedules refreshed!', isError: false);
      },
      color: theme.colorScheme.primary,
      child: Container(
        child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      ElevatedButton.icon(
                        onPressed: _scheduleTextController.text.trim().isEmpty ? null : _saveSchedule,
              ...
              child: allSchedules.when(
                data: (schedules) {
                  final filteredSchedules = schedules
                      .where((s) => _formatDate(s.scheduleDate.toDate()).contains(searchQuery))
                      .toList();
                ...
                error: (e, s) => Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.error_outline, size: 64, color: theme.colorScheme.error),
                      const SizedBox(height: 16),
                      Text('Error: $e', style: GoogleFonts.inter(color: theme.colorScheme.error)),
                    ],
                  ),
                ),
              ),
                  ...
                  Column(
                    children: [
                      ElevatedButton.icon(
                        ...
                        label: Text(
                          _isUploadingQr ? 'Uploading...' : 'Upload QR',
                        ),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: _isUploadingQr ? Colors.grey : theme.colorScheme.primary,
                          foregroundColor: Colors.white,
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            labelStyle: GoogleFonts.inter(
              color: (isDark ? Colors.white : const Color(0xFF1F2937)).withValues(alpha: isEnabled ? 0.7 : 0.4),
            ),
            hintStyle: GoogleFonts.inter(
              color: (isDark ? Colors.white : const Color(0xFF1F2937)).withValues(alpha: isEnabled ? 0.5 : 0.3),
            ),
          ),
        ),
      ],
...
class _SongFormDialogState extends ConsumerState<_SongFormDialog> {
  ...
  Future<void> _saveSong() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Song ${widget.song == null ? 'created' : 'updated'} successfully!'),
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                ...
                validator: (v) =>
                    v?.trim().isEmpty ?? true ? 'Song name is required' : null,
              ),
              const SizedBox(height: 16),
              TextFormField(
                ...
                maxLines: 6,
                validator: (v) =>
                    v?.trim().isEmpty ?? true ? 'Lyrics are required' : null,
              ),
              const SizedBox(height: 16),
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: _isLoading
              ...
              : Text(widget.song == null ? 'Add Song' : 'Update Song'),
        ),
      ],
    );
  }
